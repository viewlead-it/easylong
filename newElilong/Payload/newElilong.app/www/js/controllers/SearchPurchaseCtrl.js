CONTROLLERS.controller('SearchPurchaseCtrl', function($scope, $state, $filter, $http, ionicDatePicker) {
  $scope.records = []
  var ipObj1 = {
  ...
  function getPurchaseLists(selectDate) {
    ...
    return $http.get(SERVER_CONFIG.PROTOCOL + SERVER_CONFIG.NEWSERVICE + 'PurchaseMaster', {
      params: {
        purchaseDate:{
    ...
    .then(function(result){
      ...
      for (var idx in result.data) {
        var tmpDetail = result.data[idx].PurchaseDetails
        var weight = 0;
